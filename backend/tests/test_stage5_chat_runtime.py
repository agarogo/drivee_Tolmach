...
class Stage5ChatApiTests(unittest.TestCase):
    ...
    def test_run_query_endpoint_returns_controlled_llm_timeout_and_persists_error_message(self) -> None:
        chat_id = uuid4()
        chat = SimpleNamespace(id=chat_id, title="Revenue by city", updated_at=datetime.utcnow())
        ...
        with (
            ...
            patch("app.api.current_llm_device_hint", new=AsyncMock(return_value="cpu")),
        ):
            response = self.client.post(
                "/queries/run",
                json={"question": "show revenue", "chat_id": str(chat_id)},
            )

        self.assertEqual(response.status_code, 504)
        payload = response.json()
        self.assertEqual(payload["error_code"], "llm_timeout")
        self.assertEqual(payload["provider"], "ollama")
        self.assertEqual(payload["model"], "qwen3:4b")
        self.assertEqual(payload["device_hint"], "cpu")
        assistant_messages = [item for item in self.db.added if getattr(item, "role", None) == "assistant"]
        self.assertEqual(len(assistant_messages), 1)
        self.assertEqual(assistant_messages[0].payload["type"], "llm_error")

    def test_run_query_timeout_does_not_touch_expired_chat_after_rollback(self) -> None:
        chat_id = uuid4()

        class ExpiringChat:
            def __init__(self, value):
                self._id = value
                self.title = "Revenue by city"
            ...
            def id(self):
                if self._expired:
                    raise AssertionError("chat.id accessed after rollback")
                return self._id

            def expire(self) -> None:
                self._expired = True

        class RollbackExpiringDb(ApiDb):
            def __init__(self, expiring_chat):
                super().__init__(scalar_values=[1])
                self.expiring_chat = expiring_chat

            async def rollback(self):
                self.expiring_chat.expire()

        chat = ExpiringChat(chat_id)
        self.db = RollbackExpiringDb(chat)
        ...
        with (
            ...
            patch("app.api.current_llm_device_hint", new=AsyncMock(return_value="gpu")),
        ):
            response = self.client.post(
                "/queries/run",
                json={"question": "show revenue", "chat_id": str(chat_id)},
...
class Stage5ChatContinuityTests(unittest.IsolatedAsyncioTestCase):
    async def test_orchestrator_uses_chat_history_context_before_classification(self) -> None:
        ...
        with (
            ...
            patch("app.ai.orchestrator.resolve_answer_type_decision", new=AsyncMock(return_value=decision)),
