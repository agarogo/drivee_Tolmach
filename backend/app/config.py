...
class Settings(BaseSettings):
    ...
    # Single PostgreSQL DSN used by the whole application.
    # Keep PLATFORM_DATABASE_URL / ANALYTICS_DATABASE_URL as legacy fallbacks only so older
    # local .env files do not crash immediately, but the runtime uses one database_url.
    database_url: str = ""
    ...
    query_workflow_timeout_seconds: int = 120
    health_report_llm_timeout_seconds: float = 1.5
    ...
    def database_label(self) -> str:
        return _database_name_from_dsn(self.database_url)
    ...
    def normalize_database_urls(self) -> "Settings":
        raw_dsn = (
            ...
            or self.platform_database_url.strip()
            or self.analytics_database_url.strip()
        )
        self.database_url = _normalize_postgres_dsn(raw_dsn, "DATABASE_URL")

        # Compatibility surface for older modules/tests: both names intentionally point
        # to the exact same DSN. The product uses one database only.
        self.platform_database_url = self.database_url
