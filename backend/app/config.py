...
class Settings(BaseSettings):
    ...
    @property
    def session_secret_key_effective(self) -> str:
        if self.session_secret_key:
            return self.session_secret_key
        return "dev-only-change-me-tolmach-session-secret"
    ...
    def normalize_database_urls(self) -> "Settings":
        ...
        if self.is_production and not self.session_secret_key:
            raise ValueError("SESSION_SECRET_KEY must be set when APP_ENV=production")
