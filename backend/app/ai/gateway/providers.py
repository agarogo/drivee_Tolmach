    ...
    *,
    ...
    if isinstance(exc, LLMProviderError):
        return exc.with_context(
            provider=provider_name or exc.provider,
            model=model_name or exc.model,
    ...
    if isinstance(exc, httpx.HTTPStatusError):
        ...
        return LLMProviderError(
            f"{provider_name} returned HTTP {status_code}.",
    ...
    async def generate_structured(
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                last_error = _classify_http_error(
                    exc,
        ...
        if last_error is None:
            last_error = LLMProviderError(
                f"{self.provider_name} request failed without a captured error.",
                provider=self.provider_name,
                model=self.model_name,
            )
