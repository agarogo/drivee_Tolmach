...
class FallbackRuleProvider:
    ...
    def _summary_payload(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        metric_value = first.get(metric_key)
        leading_dimension = next((key for key in first.keys() if key != metric_key), None)
        highlights = [f"Row count: {len(rows)}"]
        if metric_value is not None:
            highlights.append(f"Top visible {metric_label}: {metric_value}")
        if leading_dimension and first.get(leading_dimension) is not None:
            highlights.append(f"Leading {leading_dimension}: {first.get(leading_dimension)}")
        return AnswerSummaryDraft(
