...
async def list_templates(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)) -> list[TemplateOut]:
    rows = list((await db.scalars(select(Template).where(or_(Template.is_public.is_(True), Template.created_by == user.id)).order_by(Template.category.asc(), Template.use_count.desc(), Template.title.asc()))).all())
