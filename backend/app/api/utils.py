...
from app.reports import list_report_runs
from app.repositories.chats import make_chat_title
...
def device_hint(request: Request) -> str:
    user_agent = (request.headers.get("user-agent") or "").strip()
    client_host = request.client.host if request.client else ""
    return " | ".join(part for part in (user_agent[:200], client_host) if part)[:255]


# Backward-compatible alias for existing route code and tests.
_device_hint = device_hint
...
def query_ai_telemetry(item: Query) -> dict[str, Any]:
...
async def run_query_workflow_or_503(
    db: AsyncSession,
    user: User,
    question: str,
    chat_id,
...
# Backward-compatible alias for existing route code and tests.
_run_query_workflow_or_503 = run_query_workflow_or_503
...
async def query_to_out(db: AsyncSession, item: Query) -> QueryOut:
    ai_telemetry = query_ai_telemetry(item)
    events = list(
        (
            await db.scalars(
