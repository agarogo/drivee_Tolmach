...
from app.llm_runtime import build_health_payload, build_llm_error_payload, current_llm_device_hint
...
async def _run_query_workflow_or_503(
    ...
    try:
        return await asyncio.wait_for(
            run_query_workflow(db, user, question, chat_id),
            timeout=max(5, settings.query_workflow_timeout_seconds),
        )
    except asyncio.TimeoutError as exc:
        await db.rollback()
        logger.error(
            "Query workflow timed out after %s seconds: chat_id=%s question=%r",
            settings.query_workflow_timeout_seconds,
            chat_id,
            question[:200],
        )
        raise LLMProviderError(
            "LLM не успела ответить за лимит времени. Проверь GPU/Ollama или упрости запрос.",
        ...
        raise exc.with_context(
            provider=exc.provider or settings.llm_provider,
            model=exc.model or settings.llm_model,
        ) from exc


async def _llm_error_payload(exc: LLMProviderError) -> dict[str, Any]:
    error_code = exc.error_code if exc.error_code in {"llm_timeout", "llm_unavailable"} else "llm_unavailable"
    provider = exc.provider or settings.llm_provider
    model = exc.model or settings.llm_model
    device_hint = await current_llm_device_hint()
    default_message = (
        "LLM не успела ответить за лимит времени. Проверь GPU/Ollama или упрости запрос."
        if error_code == "llm_timeout"
        else "LLM сейчас недоступна или вернула ошибку. Проверь GPU/Ollama и повтори запрос."
    )
    return build_llm_error_payload(
...
def assistant_payload_from_llm_error(error_payload: dict[str, Any]) -> dict[str, Any]:
    return {
        **error_payload,
        "type": "llm_error",
        "actions": ["Повторить", "Вернуть вопрос в поле ввода"],
    }


async def persist_llm_error_message(
    db: AsyncSession,
    chat_id: UUID | None,
    ...
    db.add(
        Message(
            chat_id=chat_id,
            role="assistant",
            content=str(error_payload.get("body") or error_payload.get("message") or "Не удалось получить ответ."),
            payload=assistant_payload_from_llm_error(error_payload),
        )
    )
    await db.commit()


def llm_error_response(error_payload: dict[str, Any]) -> JSONResponse:
    return JSONResponse(status_code=status.HTTP_504_GATEWAY_TIMEOUT, content=error_payload)
...
async def append_chat_message(
    ...
    *,
    ...
    message = Message(chat_id=chat.id, role=role, content=content, payload=payload)
...
async def admin_health(_: User = Depends(require_admin)) -> dict:
    payload = await build_health_payload(
        app_name="Толмач by Drivee",
...
async def run_query(
    ...
    await append_chat_message(db, chat, role="user", content=question, payload={})
    await db.commit()

    try:
        item = await _run_query_workflow_or_503(db, user, question, chat_id)
    except LLMProviderError as exc:
        error_payload = await _llm_error_payload(exc)
        await persist_llm_error_message(db, chat_id, error_payload)
        return llm_error_response(error_payload)
...
async def clarify_query(
    ...
    if original.chat_id:
        ...
        await append_chat_message(
            ...
            role="user",
            content=answer,
            payload={"clarifies_query_id": str(original.id)},
        )
    await db.commit()

    try:
        item = await _run_query_workflow_or_503(db, user, clarified_question, original_chat_id)
    except LLMProviderError as exc:
        error_payload = await _llm_error_payload(exc)
        await persist_llm_error_message(db, original_chat_id, error_payload)
        return llm_error_response(error_payload)
    output = await query_to_out(db, item)
    if chat:
        await append_chat_message(
...
async def send_message(
    ...
    if prior_count == 0:
        chat.title = make_chat_title(question)
    user_message = await append_chat_message(db, chat, role="user", content=question, payload={})
    await db.commit()
