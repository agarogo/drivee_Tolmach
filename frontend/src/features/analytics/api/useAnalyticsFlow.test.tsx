...
function makeLlmErrorMessagesPage(chatId: string, question: string) {
  return {
    items: [
      {
        id: `user-error-${chatId}`,
        chat_id: chatId,
        role: "user",
        content: question,
        payload: {},
      ...
      {
        id: `assistant-error-${chatId}`,
        chat_id: chatId,
        role: "assistant",
        content: "LLM не успела ответить за лимит времени. Проверь GPU/Ollama или упрости запрос.",
        payload: {
          type: "llm_error",
          status: "llm_timeout",
          ...
          message: "LLM не успела ответить за лимит времени. Проверь GPU/Ollama или упрости запрос.",
          provider: "ollama",
          model: "qwen3:4b",
          device_hint: "cpu",
          fallback_used: false,
...
describe("useAnalyticsFlow", () => {
  ...
  it("restores a controlled llm timeout into the current chat instead of dropping the pending question", async () => {
    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false }, mutations: { retry: false } } });
    fetchChatMessagesMock.mockResolvedValue(makeLlmErrorMessagesPage("chat-1", "show revenue"));
    ...
    act(() => {
      result.current.setDraft("show revenue");
    });
    act(() => {
      result.current.submitQuery();
    });
    ...
    await waitFor(() => expect(runQueryMock).toHaveBeenCalled());
    await waitFor(() => expect(result.current.pendingQuestion).toBe(""));
    expect(fetchChatMessagesMock).toHaveBeenLastCalledWith("chat-1");
    expect(result.current.messages.at(-1)?.payload.type).toBe("llm_error");
  });
